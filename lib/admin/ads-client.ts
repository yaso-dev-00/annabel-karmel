...
export async function fetchAds(): Promise<SiteAd[]> {
  ...
  if (!res.ok)
    throw new Error(await readApiError(res, 'Failed to fetch advertisements'));
...
export async function fetchAd(id: string): Promise<SiteAd> {
  ...
  if (!res.ok)
    throw new Error(await readApiError(res, 'Advertisement not found'));
...
export async function createAdApi(
  ...
  if (!res.ok)
    throw new Error(await readApiError(res, 'Failed to create advertisement'));
  return (await res.json()) as SiteAd;
}
...
export async function updateAdApi(
  ...
  if (!res.ok)
    throw new Error(await readApiError(res, 'Failed to update advertisement'));
  return (await res.json()) as SiteAd;
}
