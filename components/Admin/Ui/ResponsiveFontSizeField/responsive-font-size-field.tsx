...
const BREAKPOINT_KEYS: Record<
  ...
  keyof Pick<BlockSettings, "font_size_mobile" | "font_size_tablet" | "font_size_desktop">
> = {
...
function isPresetValue(value: string): boolean {
  return FONT_SIZE_PRESETS.some((preset) => preset.value && preset.value === value);
}
        ...
        return (
          ...
          >
            <div className={styles.rowHeader}>
              <span className={styles.deviceLabel}>{DS_GRID[breakpoint].label}</span>
              <span className={styles.deviceHint}>{breakpointHint(breakpoint)}</span>
            </div>
            <select
              ...
              onChange={(e) => {
                ...
                if (next === "custom") {
                  onChange({ [key]: isCustom ? value : CUSTOM_FONT_SIZE_STARTER });
                  return;
                }
                onChange({ [key]: next });
              }}
