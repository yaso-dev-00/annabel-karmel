...
export async function fetchPartners(): Promise<PartnerPage[]> {
  ...
  if (!res.ok)
    throw new Error(await readApiError(res, 'Failed to fetch partners'));
...
export async function fetchPartnerPage(id: string): Promise<PartnerPage> {
  ...
  if (!res.ok)
    throw new Error(await readApiError(res, 'Partner page not found'));
...
export async function createPartnerPageApi(
  ...
  if (!res.ok)
    throw new Error(await readApiError(res, 'Failed to create partner page'));
  return (await res.json()) as PartnerPage;
}
...
export async function updatePartnerPageApi(
  ...
  if (!res.ok)
    throw new Error(await readApiError(res, 'Failed to update partner page'));
  return (await res.json()) as PartnerPage;
}
...
export async function deletePartnerPageApi(id: string): Promise<void> {
  ...
  if (!res.ok)
    throw new Error(await readApiError(res, 'Failed to delete partner page'));
}
