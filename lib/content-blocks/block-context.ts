...
export type ContentEditorContext =
  'advice' | 'article' | 'competition' | 'partners';
export type ContentRenderContext =
  'advice' | 'article' | 'competition' | 'partners';

export const RELATED_ARTICLES_EDITOR_CONTEXTS = [
  'advice',
  'article',
] as const satisfies readonly ContentEditorContext[];

export const ADVICE_ARTICLE_ONLY_BLOCKS = [
  'related_articles',
] as const satisfies readonly BlockType[];
export const PARTNERS_ONLY_BLOCKS = [
  'partnership_tag',
] as const satisfies readonly BlockType[];

export type AdviceArticleOnlyBlockType =
  (typeof ADVICE_ARTICLE_ONLY_BLOCKS)[number];
export type PartnersOnlyBlockType = (typeof PARTNERS_ONLY_BLOCKS)[number];

export function isAdviceArticleOnlyBlock(
  type: BlockType,
): type is AdviceArticleOnlyBlockType {
  return (ADVICE_ARTICLE_ONLY_BLOCKS as readonly BlockType[]).includes(type);
}

export function isPartnersOnlyBlock(
  type: BlockType,
): type is PartnersOnlyBlockType {
  return (PARTNERS_ONLY_BLOCKS as readonly BlockType[]).includes(type);
}

export function isBlockAllowedInEditor(
  type: BlockType,
...
export function shouldRenderBlock(
  type: BlockType,
  context: ContentRenderContext = 'competition',
): boolean {
  if (isPartnersOnlyBlock(type)) return context === 'partners';
...
export function editorContextToRenderContext(
