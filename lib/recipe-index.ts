...
export async function getRecipeListingIndex(): Promise<Map<string, RecipeListingItem>> {
  if (!slugIndexPromise) {
...
export async function getRecipeListingStub(slug: string): Promise<RecipeListingItem | null> {
  const index = await getRecipeListingIndex();
