...
function resolveBorderStyle(settings?: BlockSettings): BorderStyle | undefined {
  ...
  if (settings.border_width?.trim() || hasIndividualBorder(settings) || settings.border_color?.trim()) {
    return "solid";
  }
  return undefined;
}
...
export function applyBlockBorderStyle(style: CSSProperties, settings?: BlockSettings): void {
  if (!settings) return;
  ...
  if (hasIndividualBorder(settings)) {
    ...
    if (top) style.borderTop = `${normalizeCssLength(top)} ${borderStyle} ${color}`;
    if (right) style.borderRight = `${normalizeCssLength(right)} ${borderStyle} ${color}`;
    if (bottom) style.borderBottom = `${normalizeCssLength(bottom)} ${borderStyle} ${color}`;
    if (left) style.borderLeft = `${normalizeCssLength(left)} ${borderStyle} ${color}`;
    return;
  }
