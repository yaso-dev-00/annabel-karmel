...
import { SiteNavOverlay } from '@/components/SiteLayout/SiteNavOverlay';
import { SiteAdPlacement } from '@/components/SiteLayout/SiteAdPlacement';
import { SiteNewsletter } from '@/components/SiteLayout/SiteNewsletter';
...
export function SiteHeader() {
  ...
  const nestedCloseTimerRef = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  const updatePanelAnchor = useCallback((menuLabel: string | null) => {
    ...
    const margin = 16;
    const panel =
      headerRef.current?.querySelector<HTMLElement>('.nav-overlay-panel');
    ...
    menus.map((menu) => {
      ...
      return (
        <div
          className="menu-item"
          key={menu.label}
          onMouseEnter={() => openDesktopNav(menu.label)}
        >
          <button
            type="button"
            ref={(element) => {
                ...
                return (
                  <div key={menu.label} className="border-b border-[#b9afb3]">
                    ...
                    >
                      <span>{menu.label}</span>
                      <span
                        className={`text-[15px] text-[#6a6368] ${isOpen ? 'rotate-180' : 'translate-y-px'}`}
                        aria-hidden
                      >
                        ▾
                      </span>
                        ...
                        {menu.layout === 'dropdown'
                          ? menu.groups.flatMap((group) =>
                              group.links.map((link) => {
                                ...
                                const isNestedOpen =
                                  mobileOpenSubGroups.includes(nestedKey);

                                if (link.children?.length) {
                                  return (
                                    <div
                                      key={link.label}
                                      className="border-t border-[#b9afb3]"
                                    >
                                      <button
                                        type="button"
                                        onClick={() =>
                                          toggleMobileSubGroup(nestedKey)
                                        }
                                        ...
                                        <ul className="m-0 list-none overflow-hidden p-0 pb-[0.35rem]">
                                          {link.children.map((child) => {
                                            ...
                                            return (
                                              <li
                                                key={child.label}
                                                className="border-t border-[#d4cdd0]"
                                              >
                                                <Link
                                                  href={child.href}
                                                  onClick={() =>
                          ...
                          : menu.groups.map((group) => {
                              ...
                              const isSubOpen =
                                mobileOpenSubGroups.includes(subGroupKey);
                              return (
                                <div
                                  key={group.title}
                                  className="border-t border-[#b9afb3]"
                                >
                                  <button
                                    type="button"
                                    onClick={() =>
                                      toggleMobileSubGroup(subGroupKey)
                                    }
