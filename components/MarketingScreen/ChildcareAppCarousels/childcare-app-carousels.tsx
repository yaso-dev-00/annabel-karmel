...
import {
  childcareExpertBenefits,
  childcareRecipeCategories,
...
import styles from "@/components/MarketingScreen/ChildcareAppPage/childcare-app-page.module.css";
...
const EXPERT_LOGICAL_COUNT = childcareExpertBenefits.length;
...
export function ChildcareRecipeCarousel() {
  const items = [...childcareRecipeCategories, ...childcareRecipeCategories];
  ...
  const [remainingCount, setRemainingCount] = useState(childcareRecipeCategories.length);
  ...
  const updateProgress = useCallback((offset: number) => {
    ...
    setRemainingCount(
      Math.max(0, Math.ceil((1 - normalized) * childcareRecipeCategories.length)),
    );
  }, []);
          ...
          {items.map((item, index) => (
            ...
            >
              <Image
                ...
                sizes={childcareImageSizes.recipe}
                draggable={false}
...
export function ChildcareExpertCarousel() {
        ...
        {loopItems.map((item, index) => (
          ...
          >
            <Image
              ...
              alt=""
              fill
              sizes={childcareImageSizes.expert}
