...
export async function fetchCookbooks(): Promise<Cookbook[]> {
  ...
  if (!res.ok)
    throw new Error(await readApiError(res, 'Failed to fetch cookbooks'));
...
export async function createCookbookApi(
  ...
  if (!res.ok)
    throw new Error(await readApiError(res, 'Failed to create cookbook'));
  return (await res.json()) as Cookbook;
}
...
export async function updateCookbookApi(
  ...
  if (!res.ok)
    throw new Error(await readApiError(res, 'Failed to update cookbook'));
  return (await res.json()) as Cookbook;
}
...
export async function deleteCookbookApi(id: string): Promise<void> {
  ...
  if (!res.ok)
    throw new Error(await readApiError(res, 'Failed to delete cookbook'));
}
