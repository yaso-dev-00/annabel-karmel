      ...
      <main className="overflow-x-hidden bg-white">
        <article className="mx-auto mt-[30px] w-full max-w-[1200px] px-[8px] pb-[10px] pt-[20px] md:px-[14px] md:pt-[28px]">
          ...
          <p className={styles.body}>
            As well as enthusiasm, some basic weaning equipment can help you on your weaning way and the following items
            might make life easier:
          </p>

          {equipmentItems.map((item) => (
            <p key={item.label} className={styles.equipmentItem}>
              <strong className={styles.itemLabel}>{item.label}</strong> {item.text}
            </p>
          ))}

          <p className={styles.equipmentItem}>
            <strong className={styles.itemLabel}>Freezing trays:</strong> Your little one is likely to have only one or
            two teaspoons to start with and making such a small quantity each time is fiddly and not cost-effective.
