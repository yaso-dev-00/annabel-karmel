...
export function isBlobArticlesCmsStoreEnabled(): boolean {
...
async function readBlobRaw(): Promise<string | null> {
  if (!isBlobArticlesCmsStoreEnabled()) return null;
...
async function writeBlobRaw(raw: string): Promise<void> {
  if (!isBlobArticlesCmsStoreEnabled()) {
    throw new Error(
...
async function blobStoreHasData(): Promise<boolean> {
  if (!isBlobArticlesCmsStoreEnabled()) return false;
