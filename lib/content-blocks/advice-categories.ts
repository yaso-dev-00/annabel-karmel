...
export type AdviceCategorySlug = (typeof ADVICE_CATEGORY_OPTIONS)[number]["value"];

export function getAdviceCategoryLabel(categorySlug: string): string {
  return ADVICE_CATEGORY_OPTIONS.find((option) => option.value === categorySlug)?.label ?? categorySlug;
}
