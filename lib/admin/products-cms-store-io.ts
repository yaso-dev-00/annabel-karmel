...
const LEGACY_BLOB_PATHNAME = 'cms/products.json';
const VERSIONED_PREFIX = 'cms/products/v';
const MAX_VERSIONS = 12;

const CMS_DIR = path.join(process.cwd(), 'data', 'cms');
const LOCAL_RUNTIME_FILE = path.join(CMS_DIR, 'products.json');
const SEED_FILE = path.join(CMS_DIR, 'products.seed.json');
const BUNDLED_SEED_RAW = JSON.stringify(seedStore);

export function isBlobProductsCmsStoreEnabled(): boolean {
...
async function listProductsStoreBlobs() {
...
async function readBlobRaw(): Promise<string | null> {
  if (!isBlobProductsCmsStoreEnabled()) return null;
...
async function writeBlobRaw(raw: string): Promise<void> {
  if (!isBlobProductsCmsStoreEnabled()) {
    throw new Error(
...
async function blobStoreHasData(): Promise<boolean> {
  if (!isBlobProductsCmsStoreEnabled()) return false;
  const blobs = await listProductsStoreBlobs();
  return blobs.length > 0;
}

export async function readProductsCmsStoreRaw(): Promise<string> {
  if (isBlobProductsCmsStoreEnabled()) {
...
export async function writeProductsCmsStoreRaw(raw: string): Promise<void> {
  if (isBlobProductsCmsStoreEnabled()) {
