...
function getPreviewLayout(width: number, perDesktopView: number, itemCount: number): CarouselLayout {
...
function getPreviewCardWidth(viewportWidth: number, layout: CarouselLayout, gap: number) {
...
export function ArticleRecipeCarouselPreview({
  ...
  const canCycle = items.length > 1 && carousel.maxIndex > 0;
  const layout = getPreviewLayout(viewportWidth || 320, perDesktopView, items.length);
  const computedCardWidth =
    viewportWidth > 0 ? getPreviewCardWidth(viewportWidth, layout, gap) : null;

  const measureCards = useCallback(() => {
    const viewport = carousel.carouselRef.current;
    ...
    carousel.measure();
  }, [carousel, gap, items.length, perDesktopView]);
  ...
  }, [carousel.carouselRef, measureCards]);

  const handleNavigation = useCallback(
    (direction: number) => {
      if (!canCycle) return;
      if (loop) {
        if (direction > 0 && carousel.isAtEnd) {
          carousel.animateToIndex(0, CAROUSEL_SMOOTH);
          return;
        }
        if (direction < 0 && carousel.isAtStart) {
          carousel.animateToIndex(carousel.maxIndex, CAROUSEL_SMOOTH);
          return;
        }
      }
      carousel.handleNavigation(direction);
    },
    [canCycle, carousel, loop],
  );

  const cardStyle =
    computedCardWidth && computedCardWidth > 0 ? { width: computedCardWidth } : undefined;
    ...
    >
      ...
      >
        ...
        <button
          ...
          disabled={!canCycle || (!loop && carousel.isAtStart)}
          onPointerDown={(event) => event.stopPropagation()}
        ...
        <button
          ...
          disabled={!canCycle || (!loop && carousel.isAtEnd)}
          onPointerDown={(event) => event.stopPropagation()}
          ...
          {items.map((recipe, index) => (
            <button
              ...
              className={`h-2.5 w-2.5 rounded-full border-0 p-0 transition-colors ${
                carousel.index === index ? "bg-[#b34769]" : "bg-[#efcfd8]"
              }`}
              onClick={(event) => {
                event.stopPropagation();
                carousel.animateToIndex(index, CAROUSEL_SMOOTH);
              }}
            />
          ))}
        </div>
