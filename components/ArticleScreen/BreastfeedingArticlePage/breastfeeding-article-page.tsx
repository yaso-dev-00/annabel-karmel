...
export const milkMakingMamaInstagramUrl = "https://www.instagram.com/milkmakingmama/";
 ...
 */
export type ArticleStyles = Partial<Record<
  | "body"
  | "bodyBold"
  | "bodyBoldItalic"
  | "sectionTitle"
...
export type BreastfeedingBlock =
  | { type: "paragraph"; content: BreastfeedingInline[]; bold?: boolean; italic?: boolean }
  ...
  | { type: "relatedLinks"; intro: string; links: { label: string; href: string }[] }
...
function BreastfeedingBlockView({ block, styles }: { block: BreastfeedingBlock; styles: StyleMap }) {
  switch (block.type) {
    case "paragraph":
      return (
        <p className={paragraphClassName(block, styles)}>{renderInline(block.content, styles)}</p>
      );
    case "heading":
    ...
    case "divider":
      return block.src ? (
        // eslint-disable-next-line @next/next/no-img-element
        <img className={styles.divider} src={block.src} alt="" aria-hidden="true" loading="lazy" />
      ) : (
