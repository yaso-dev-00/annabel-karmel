...
const LEGACY_BLOB_PATHNAME = 'cms/partners.json';
const VERSIONED_PREFIX = 'cms/partners/v';
const MAX_VERSIONS = 12;

const CMS_DIR = path.join(process.cwd(), 'data', 'cms');
const LOCAL_RUNTIME_FILE = path.join(CMS_DIR, 'partners.json');
const SEED_FILE = path.join(CMS_DIR, 'partners.seed.json');
const BUNDLED_SEED_RAW = JSON.stringify(seedStore);

export function isBlobPartnersCmsStoreEnabled(): boolean {
...
async function listPartnersStoreBlobs() {
...
async function readBlobRaw(): Promise<string | null> {
  if (!isBlobPartnersCmsStoreEnabled()) return null;
...
async function writeBlobRaw(raw: string): Promise<void> {
  if (!isBlobPartnersCmsStoreEnabled()) {
    throw new Error(
...
async function blobStoreHasData(): Promise<boolean> {
  if (!isBlobPartnersCmsStoreEnabled()) return false;
  const blobs = await listPartnersStoreBlobs();
  return blobs.length > 0;
}

export async function readPartnersCmsStoreRaw(): Promise<string> {
  if (isBlobPartnersCmsStoreEnabled()) {
...
export async function writePartnersCmsStoreRaw(raw: string): Promise<void> {
  if (isBlobPartnersCmsStoreEnabled()) {
