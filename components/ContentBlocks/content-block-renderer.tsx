...
import {
  getImageInlineStyle,
  getImageStackItemStyle,
  getTwoColumnImageBleedStyle,
  getTwoColumnImageDisplayStyle,
} from '@/lib/admin/block-image-resize';
import {
  CmsAccordionBlock,
  CmsExpertAttributionBlock,
} from './blocks/cms-accordion-block';
...
import {
  ...
  hasParagraphGap,
  resolveColumnProseParagraphGap,
} from '@/lib/content-blocks/block-prose';
import { getImageStackGridStyle } from '@/lib/content-blocks/image-stack-columns';
import {
  getRecipeGridStyle,
  recipeGridUsesMatchAspect,
} from '@/lib/content-blocks/recipe-grid-columns';
import {
  getProductGridStyle,
  productGridUsesMatchAspect,
} from '@/lib/content-blocks/product-grid-columns';
import {
...
import {
  getRelatedLinkClass,
  getRelatedLinksListClass,
  resolveRelatedLinksStyle,
} from '@/lib/content-blocks/related-links-styles';
import {
  getTableClassNames,
  resolveTableInlineStyle,
...
function RenderBlock({
  ...
  switch (block.type) {
    case 'hero': {
      ...
      return (
        <section
          className={`${styles.hero} ${wrapClass}`}
          style={getHeroStyle(d, block.settings)}
        >
          <h1 className={styles.heroHeadline} style={{ color: d.text_color }}>
            {d.headline}
          </h1>
          {d.subheadline ? (
            <p className={styles.heroSubheadline}>{d.subheadline}</p>
          ) : null}
          {d.image_url ? (
      ...
      const cls =
        d.level === 'h1'
          ...
          : d.level === 'h3'
            ...
            : styles.sectionTitle;
      if (d.level === 'h1')
        return <h1 className={`${cls} ${wrapClass}`}>{d.text}</h1>;
      if (d.level === 'h3')
        return <h3 className={`${cls} ${wrapClass}`}>{d.text}</h3>;
    ...
    case 'image': {
      ...
      const mobileSrcRaw = isImageBlockMobileEnabled(d)
        ? resolveImageBlockMobileSrc(d)
        : undefined;
      const mobileSrc = mobileSrcRaw
        ? resolveImageSrc(mobileSrcRaw)
        : undefined;
      const useMobileSwap = Boolean(
        imageSrc && mobileSrc && mobileSrc !== imageSrc,
      );
      const sized = imageBlockHasCustomSize(d);
      const imageClass =
        `${styles.image} ${d.full_width ? styles.imageFull : ''} ${sized ? styles.imageSized : ''}`.trim();
      ...
      return (
        <figure className={wrapClass}>
          {media ? (
            d.link_href ? (
              <Link href={d.link_href}>{media}</Link>
            ) : (
              media
            )
          ) : null}
    ...
    case 'image_stack': {
      ...
      return (
        <div
          className={`${layoutCls} ${wrapClass}`}
          style={getImageStackGridStyle(d)}
        >
                ...
                {imageSrc ? (
                  ...
                  <img
                    ...
                    style={getImageStackItemStyle(img, {
                      grid: isGrid,
                      matchAspect: Boolean(matchAspect),
                    })}
                    data-cms-resize-image="true"
      ...
      return (
        <CmsAccordionBlock
          ...
          style={getIntrinsicChildBoxStyle(block.settings, {
            backgroundColor:
              block.settings?.background_color?.trim() || '#ffffff',
            clipOverflow: false,
          })}
          paragraphGapSettings={
            hasParagraphGap(block.settings) ? block.settings : undefined
          }
        />
      );
    case 'table': {
      ...
      return (
        ...
        >
          {d.caption ? (
            <p className={styles.tableCaption}>{d.caption}</p>
          ) : null}
    ...
    case 'callout': {
      ...
      return (
        <aside
          className={`${styles.callout} ${variantCls} ${wrapClass}`}
          style={getCalloutStyle(d, block.settings)}
        >
              ...
              return (
                <li key={`${link.href}-${link.label}`}>
                  <Link
                    href={link.href}
                    className={linkClass}
                    target={isRow ? '_blank' : undefined}
                    rel={isRow ? 'noreferrer' : undefined}
                  >
                    {iconSrc ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img
                        src={iconSrc}
                        alt={link.icon_alt || ''}
                        className={styles.relatedSocialIcon}
                      />
                    ) : null}
    ...
    case 'announcement_banner': {
      ...
      return (
        <div
          ...
          style={getAnnouncementBannerStyle(d, block.settings)}
        >
          {d.message}
        ...
        <figure className={wrapClass}>
          <div className="aspect-video w-full overflow-hidden rounded-lg bg-black">
            {d.provider === 'youtube' && d.url ? (
              <iframe
                src={
                  d.url.includes('embed')
                    ? d.url
                    : `https://www.youtube.com/embed/${extractYoutubeId(d.url)}`
                }
                title="Video"
    ...
    case 'form_embed': {
      ...
      return (
        <div style={formChromeStyle}>
          ...
          {d.embed_code ? (
            <div dangerouslySetInnerHTML={{ __html: d.embed_code }} />
          ) : null}
        </div>
      );
    }
    case 'partner_promo': {
      ...
      const logo = d.logo_src ? (
        ...
        <img
          src={d.logo_src}
          alt={d.logo_alt}
          className={styles.partnerPromoLogo}
        />
      ) : null;
      ...
      return (
        ...
        >
          {logo ? (
            d.logo_href ? (
              <Link href={d.logo_href}>{logo}</Link>
            ) : (
              logo
            )
          ) : null}
    ...
    case 'partnership_tag': {
      ...
      const logo = logoSrc ? (
        ...
        <img
          src={logoSrc}
          alt={d.logo_alt}
          className={styles.partnershipTagLogo}
        />
      ) : null;
      return (
        <div className={`${styles.partnershipTag} ${wrapClass}`}>
          <p className={styles.partnershipTagLabel}>
            {d.label || 'In partnership with'}
          </p>
          {logo ? (
            d.logo_href ? (
              <Link
                href={d.logo_href}
                className={styles.partnershipTagLogoLink}
              >
                {logo}
              </Link>
            ) : (
    ...
    case 'book_promo': {
      ...
      return (
        <aside
          className={`${styles.imageText} ${styles.imageTextLeft} ${wrapClass}`}
        >
    ...
    case 'two_column': {
      ...
      const renderColumn = (
        ...
        minis.map((mini, i) => {
          const imageIndex =
            mini.type === 'image' && mini.src?.trim()
              ? imageFlatIndex++
              : undefined;
          const proseGapSettings = resolveColumnProseParagraphGap(
            columnSettings,
            block.settings,
          );
          return (
      ...
      const renderColumnShell = (
        ...
        const columnSelected =
          previewMode && selectedTwoColumnTarget === column;
        const columnCustomPadding = hasCustomPadding(columnSettings);
        const isImageOnlyColumn =
          minis.length === 1 &&
          minis[0]?.type === 'image' &&
          Boolean(minis[0].src?.trim());
        const hasImage = minis.some(
          (mini) => mini.type === 'image' && mini.src?.trim(),
        );
        const columnStyle = {
      ...
      return (
        <div
          ...
          onClick={
            previewMode && onTwoColumnTargetSelect
              ? (e) => {
                  if ((e.target as HTMLElement).closest('[data-cms-column]'))
                    return;
                  e.stopPropagation();
      ...
      const sizeStyle = inTwoColumn
        ...
        : block.width || block.height
          ? {
              ...
              objectFit: (block.height ? 'cover' : undefined) as
                'cover' | undefined,
            }
          : undefined;
      const image = (
        ...
        <img
          ...
          style={sizeStyle}
          data-cms-resize-image="true"
          data-cms-image-index={
            imageIndex != null ? String(imageIndex) : undefined
          }
        />
      );
...
function getPreviewBlockWrapperStyle(
  ...
  return getBlockWrapperStyleForRender(
    block,
    contentMaxWidth,
    contentMaxWidthCustom,
  );
      ...
      {sorted.map((block) => {
        ...
        return (
          <PreviewBlockWrapper
            ...
            onDataChange={
              onBlockDataChange
                ? (data) => onBlockDataChange(block.id, data)
                : undefined
            }
          >
            <RenderBlock
