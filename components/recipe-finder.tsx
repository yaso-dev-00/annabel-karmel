...
import { RecipeFinderSelect } from "@/components/recipe-finder-select";
...
export function RecipeFinder({ taxonomy, initialFilters }: RecipeFinderProps) {
  ...
  const [freeFromSlug, setFreeFromSlug] = useState(initialFilters.freeFrom ?? "");
  ...
  const submitFilters = useCallback(
    (overrides?: Partial<RecipeSearchFilters>) => {
      const next: RecipeSearchFilters = {
        ...
        mealTime: overrides?.mealTime !== undefined ? overrides.mealTime : mealTime || undefined,
        freeFrom: overrides?.freeFrom !== undefined ? overrides.freeFrom : freeFromSlug || undefined,
    ...
    [age, freeFromSlug, mealTime, q, router, taxonomy.kind, taxonomy.slug],
  );
  ...
  const handleReset = () => {
    setQ("");
    setAge(taxonomy.kind === "recipe-category" ? taxonomy.slug : "");
    setMealTime(taxonomy.kind === "meal-time" ? taxonomy.slug : "");
    setFreeFromSlug(taxonomy.kind === "allergen" ? taxonomy.slug : "");
    setAdvancedOpen(false);
    router.push(taxonomy.path);
  };
      ...
      <div className="relative z-[1] mx-auto w-full max-w-[720px] text-center">
        {!advancedOpen ? (
          <div className="w-full">
            ...
            <form className="relative mx-auto max-w-[560px]" onSubmit={handleSearchSubmit}>
          ...
          <div className="w-full">
            ...
            <div className="mx-auto mt-6 grid max-w-[480px] gap-2.5">
              <button type="button" className={findButtonClass} onClick={() => submitFilters()}>
                FIND RECIPES
              </button>
              <button type="button" className={resetButtonClass} onClick={handleReset}>
                RESET
              </button>
            </div>
          </div>
