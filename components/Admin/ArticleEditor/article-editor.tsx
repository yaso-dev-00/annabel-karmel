...
export function ArticleEditor({ initialArticle, isNew }: ArticleEditorProps) {
  ...
  const save = async (publish = false) => {
    ...
    setMessage(null);
    try {
      const payload = buildArticleSavePayload(article, { publish });
      if (isNew || !article.id) {
  ...
  const articleStatus = resolveArticleStatus(article);
  const previewable = isArticlePreviewable(article);
        ...
        <div className="editorPageHeader">
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button
              ...
              onClick={publish}
              disabled={saving || isArticleDisabled(article)}
            >
              Publish
            </button>
          </div>
