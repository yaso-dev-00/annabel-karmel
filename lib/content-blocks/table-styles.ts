...
export function resolveTableInlineStyle(
  ...
  return {
    ["--table-header-bg" as string]: data.header_bg ?? DEFAULT_TABLE_COLORS.header_bg,
    ...
    ["--table-label-width" as string]:
      data.label_width === "35" ? "35%" : data.label_width === "45" ? "45%" : "auto",
  };
}
...
export function getTableClassNames(
  ...
  const borderStyle = data.border_style ?? (data.style === "borderless" ? "none" : "full");
