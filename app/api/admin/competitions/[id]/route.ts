...
export async function GET(_request: Request, context: RouteContext) {
  ...
  if (!competition) return NextResponse.json({ error: "Not found" }, { status: 404, headers: NO_STORE_HEADERS });
...
export async function PUT(request: Request, context: RouteContext) {
  try {
    const { id } = await context.params;
    const body = (await request.json()) as Partial<Omit<Competition, "id" | "created_at">>;
