...
export function ensureNestedMiniBlockId(
...
export function normalizeTwoColumnData(
...
export function ensureImageStackItemId(
  item: ImageStackItem,
): ImageStackItem & { id: string } {
  if (item.id) return item as ImageStackItem & { id: string };
  return { ...item, id: createBlockId() };
}

export function normalizeImageStackData(
...
export function sortBlocksByOrder(blocks: ContentBlock[]): ContentBlock[] {
  return normalizeBlockOrder(blocks.slice().sort((a, b) => a.order - b.order));
}
