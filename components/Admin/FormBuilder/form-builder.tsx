      ...
      <div className={styles.fieldCardBody}>
        ...
        {field.type === 'button' ? (
          ...
          <input
            ...
            onChange={(e) =>
              onInlineChange(patchCheckboxDisplayLabel(field, e.target.value))
            }
            onClick={(e) => e.stopPropagation()}
...
function RowCard({
  ...
  const fieldCards = row.fields.map((field) => {
    const fieldProps = {
      ...
      onInlineChange: (patch: Partial<FormField>) =>
        onPatchField(field.id, patch),
    };
    ...
    <div ref={nodeRef} style={style} className={styles.sectionCard}>
      <div className={styles.sectionHeader}>
        ...
        <input
          ...
          onChange={(e) =>
            onChange(
              updateSection(schema, section.id, { title: e.target.value }),
            )
          }
      ...
      {expanded ? (
        <>
          <textarea
            ...
            onChange={(e) =>
              onChange(
                updateSection(schema, section.id, {
...
export function FormBuilder({ schema, onChange }: FormBuilderProps) {
  ...
  const [activeTab, setActiveTab] = useState<'builder' | 'preview' | 'json'>(
    'builder',
  );
  ...
  const findFieldLocation = useCallback(
    (
      fieldId: string,
    ): { sectionId: string; rowId: string; index: number } | null => {
      for (const section of schema.sections) {
        for (const row of section.rows) {
