...
export async function GET(_request: Request, context: RouteContext) {
  ...
  if (!ad) return NextResponse.json({ error: "Not found" }, { status: 404, headers: NO_STORE_HEADERS });
...
export async function PUT(request: Request, context: RouteContext) {
  try {
    const { id } = await context.params;
    const body = (await request.json()) as Partial<Omit<SiteAd, "id" | "created_at">>;
    const ad = await updateAd(id, body);
    if (!ad) return NextResponse.json({ error: "Not found" }, { status: 404, headers: NO_STORE_HEADERS });
    return NextResponse.json(ad, { headers: NO_STORE_HEADERS });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Failed to update advertisement";
