...
import type { HomepageDocument, HomepageSection, HomepageStatus } from "@/lib/homepage/types";
...
function pinRecipeFinder(sections: HomepageSection[]): HomepageSection[] {
  ...
  const without = sections.filter((section) => section.type !== "recipe_finder");
...
export function HomepageEditor({ initialDocument }: HomepageEditorProps) {
  ...
  const handleStatusChange = async (status: AdviceArticleStatus, scheduledAt?: string | null) => {
    const next = applyHomepageStatus(document, status as HomepageStatus, scheduledAt);
    setDocument(next);
      ...
      <div className="editorPageHeader">
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {previewable ? (
            ...
            <button type="button" className="btn btnGhost" disabled title="Preview unavailable while disabled">
              Preview
            </button>
          )}
          <button type="button" className="btn btnSecondary" onClick={() => save(false)} disabled={saving}>
            Save draft
          </button>
          <button
            type="button"
      ...
      <div className={blockStyles.editorWorkspace}>
        <div className={blockStyles.editorWorkspaceMain}>
          ...
          <HomepageSectionsEditor
            ...
            onFocusSection={(type) => setFocusRequest({ type, nonce: Date.now() })}
          />
        </div>
