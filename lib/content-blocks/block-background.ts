...
export function usesIntrinsicBackgroundChildBox(
  blockType?: BlockType,
): boolean {
  return (
    blockType === 'accordion' ||
...
export function resolveCalloutDefaultBackgroundColor(
  data: CalloutBlockData,
): string {
  return (
    data.background_color?.trim() || CALLOUT_VARIANT_BACKGROUNDS[data.variant]
  );
}
...
export function resolveRichTextBackgroundColor(
  ...
  if (settings?.background_color?.trim())
    return settings.background_color.trim();
...
export function resolveToolbarBackgroundStoredValue(
  block: ContentBlock,
): string | undefined {
...
export function resolveToolbarBackgroundPresetFallback(
  block: ContentBlock,
): string | undefined {
...
export function resolveToolbarBackgroundColor(
  block: ContentBlock,
): string | undefined {
  ...
  if (block.type === 'expert_attribution') {
    if (
      block.data.preset === 'kerry_secker' ||
      block.data.preset === 'custom'
    ) {
      return '#fff2ea';
    }
    return undefined;
