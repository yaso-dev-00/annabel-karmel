...
export function resolveMaxWidth(
  ...
  if (preset === 'custom') {
    return custom?.trim()
      ? normalizeCustomMaxWidth(custom)
      : MAX_WIDTH_VALUES.default;
  }
...
export function resolveBlockMaxWidth(
  ...
  if (blockPreset !== undefined) {
    return resolveMaxWidth(
      blockPreset,
      blockPreset === 'custom' ? blockCustom : undefined,
    );
  }
  return resolveMaxWidth(
    articlePreset,
    articlePreset === 'custom' ? articleCustom : undefined,
  );
}
