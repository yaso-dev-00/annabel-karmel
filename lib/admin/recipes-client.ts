...
export async function fetchRecipes(): Promise<Recipe[]> {
  ...
  if (!res.ok)
    throw new Error(await readApiError(res, 'Failed to fetch recipes'));
...
export async function createRecipeApi(
  ...
  if (!res.ok)
    throw new Error(await readApiError(res, 'Failed to create recipe'));
  return (await res.json()) as Recipe;
}
...
export async function updateRecipeApi(
  ...
  if (!res.ok)
    throw new Error(await readApiError(res, 'Failed to update recipe'));
  return (await res.json()) as Recipe;
}
...
export async function deleteRecipeApi(id: string): Promise<void> {
  ...
  if (!res.ok)
    throw new Error(await readApiError(res, 'Failed to delete recipe'));
}
