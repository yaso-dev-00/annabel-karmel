...
const LEGACY_BLOB_PATHNAME = 'cms/ads.json';
const VERSIONED_PREFIX = 'cms/ads/v';
const MAX_VERSIONS = 12;

const CMS_DIR = path.join(process.cwd(), 'data', 'cms');
const LOCAL_RUNTIME_FILE = path.join(CMS_DIR, 'ads.json');
const SEED_FILE = path.join(CMS_DIR, 'ads.seed.json');
const BUNDLED_SEED_RAW = JSON.stringify(seedStore);

export function isBlobAdsCmsStoreEnabled(): boolean {
...
async function listAdsStoreBlobs() {
...
async function readBlobRaw(): Promise<string | null> {
  if (!isBlobAdsCmsStoreEnabled()) return null;
...
async function blobStoreHasData(): Promise<boolean> {
  if (!isBlobAdsCmsStoreEnabled()) return false;
  const blobs = await listAdsStoreBlobs();
  return blobs.length > 0;
}

export async function readAdsCmsStoreRaw(): Promise<string> {
  if (isBlobAdsCmsStoreEnabled()) {
...
export async function writeAdsCmsStoreRaw(raw: string): Promise<void> {
  if (isBlobAdsCmsStoreEnabled()) {
