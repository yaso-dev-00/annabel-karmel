...
const LEGACY_BLOB_PATHNAME = 'cms/competitions.json';
const VERSIONED_PREFIX = 'cms/competitions/v';
const MAX_VERSIONS = 12;

const CMS_DIR = path.join(process.cwd(), 'data', 'cms');
const LOCAL_RUNTIME_FILE = path.join(CMS_DIR, 'competitions.json');
const SEED_FILE = path.join(CMS_DIR, 'competitions.seed.json');
const BUNDLED_SEED_RAW = JSON.stringify(seedStore);

export function isBlobCompetitionsCmsStoreEnabled(): boolean {
...
async function listCompetitionsStoreBlobs() {
...
async function readBlobRaw(): Promise<string | null> {
  if (!isBlobCompetitionsCmsStoreEnabled()) return null;
...
async function writeBlobRaw(raw: string): Promise<void> {
  if (!isBlobCompetitionsCmsStoreEnabled()) {
    throw new Error(
...
async function blobStoreHasData(): Promise<boolean> {
  if (!isBlobCompetitionsCmsStoreEnabled()) return false;
  const blobs = await listCompetitionsStoreBlobs();
  return blobs.length > 0;
}

export async function readCompetitionsCmsStoreRaw(): Promise<string> {
  if (isBlobCompetitionsCmsStoreEnabled()) {
...
export async function writeCompetitionsCmsStoreRaw(raw: string): Promise<void> {
  if (isBlobCompetitionsCmsStoreEnabled()) {
