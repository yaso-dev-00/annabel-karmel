...
export const GRID_IMAGE_ASPECT_OPTIONS: {
...
}[] = [
  {
    value: 'auto',
    label: 'Natural',
    hint: 'Each image keeps its own shape (best for tip graphics)',
  },
  {
    value: '4/3',
    label: 'Match 4:3',
    hint: 'Even row height — good for photos',
  },
...
export const RESPONSIVE_GRID_COLUMN_OPTIONS: ResponsiveGridColumnCount[] = [
  1, 2, 3, 4,
];
...
export function resolveGridImageAspect(
  value?: GridImageAspect,
): GridImageAspect {
  if (
    value === '4/3' ||
...
export function withResponsiveGridColumnDefaults<
  T extends ResponsiveGridColumns,
>(data: T): T {
...
export function getResponsiveGridStyle(
  data: ResponsiveGridColumns,
): CSSProperties {
  const columns = resolveResponsiveGridColumns(data);
  ...
  if (aspect !== 'auto') {
    (style as Record<string, string>)['--cms-grid-image-aspect'] =
      aspect.replace('/', ' / ');
  }
  return style;
}
