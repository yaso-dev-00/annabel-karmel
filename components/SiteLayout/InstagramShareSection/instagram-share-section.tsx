        ...
        <div className="flex w-max gap-2 px-[30px] sm:grid sm:w-full sm:grid-cols-2 sm:gap-2 sm:px-0 md:grid-cols-3 lg:grid-cols-6">
          {instagramPostCards.map((post, index) => (
            ...
            >
              ...
              />
              {post.kind === "carousel" ? (
                <span className="pointer-events-none absolute right-2 top-2 drop-shadow-[0_1px_4px_rgba(0,0,0,0.35)]" aria-hidden>
                  <svg viewBox="0 0 26 26" width="26" height="26" className="text-white" fill="none">
                    <rect x="3" y="3" width="15" height="15" rx="2.5" fill="rgba(0,0,0,0.22)" stroke="currentColor" strokeWidth="2" />
                    <rect x="9" y="9" width="15" height="15" rx="2.5" fill="rgba(0,0,0,0.35)" stroke="currentColor" strokeWidth="2" />
                  </svg>
                </span>
              ) : null}
              {post.kind === "video" ? (
                <span className="pointer-events-none absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-white drop-shadow-[0_3px_16px_rgba(0,0,0,0.45)]" aria-hidden>
                  <svg viewBox="0 0 64 72" width="64" height="72" className="fill-current" aria-hidden>
                    <path
