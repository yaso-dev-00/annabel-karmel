...
export function useBlobCompetitionsCmsStore(): boolean {
...
async function readBlobRaw(): Promise<string | null> {
  if (!useBlobCompetitionsCmsStore()) return null;
...
async function writeBlobRaw(raw: string): Promise<void> {
  if (!useBlobCompetitionsCmsStore()) {
    throw new Error(
...
async function blobStoreHasData(): Promise<boolean> {
  if (!useBlobCompetitionsCmsStore()) return false;
