...
export function CompetitionEditor({ initialCompetition, isNew }: CompetitionEditorProps) {
  ...
  const update = useCallback(<K extends keyof Competition>(key: K, value: Competition[K]) => {
  ...
  const handleStatusChange = async (status: CompetitionStatus, scheduledAt?: string | null) => {
    ...
    if (!isNew && competition.id) {
      ...
      setMessage(null);
      try {
        const updated = await updateCompetitionApi(competition.id, getCompetitionStatusPatch(next));
        setCompetition(updated);
        setDirty(false);
        setMessage(
          status === "disabled"
      ...
      <div className="editorSections">
        <div className="editorPageHeader">
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button type="button" className="btn btnPrimary" onClick={publish} disabled={saving || isCompetitionDisabled(competition)}>
              Publish
            </button>
          </div>
        </div>
            ...
            <div className="card">
              ...
              <div className="metaGrid">
                <div className="field">
                  ...
                  <input className="fieldInput" value={competition.title} onChange={(e) => update("title", e.target.value)} />
                </div>
                <div className="field">
                  <label className="fieldLabel">Slug</label>
                  <input className="fieldInput" value={competition.slug} onChange={(e) => update("slug", e.target.value)} placeholder="win-an-amazing-prize" />
                </div>
                <div className="field">
                  ...
                  <input
                    ...
                    type="datetime-local"
                    value={competition.closes_at ? competition.closes_at.slice(0, 16) : ""}
                    onChange={(e) =>
                      update("closes_at", e.target.value ? new Date(e.target.value).toISOString() : null)
                    }
                  />
                </div>
                <div className="field">
                  <label className="fieldLabel" htmlFor="competition-content-max-width">
                    Content max width
                  </label>
                  <MaxWidthField
            ...
            <div className="card">
              ...
              <div className="cardForm">
                <div className="field">
                  ...
                  <input className="fieldInput" value={competition.seo_title} onChange={(e) => update("seo_title", e.target.value)} />
                </div>
                <div className="field">
                  <label className="fieldLabel">SEO description</label>
                  <textarea className="fieldTextarea" value={competition.seo_description} onChange={(e) => update("seo_description", e.target.value)} rows={3} />
                </div>
              </div>
            </div>
            ...
            <div className="card">
              ...
              <div className="cardForm">
                {competition.content_blocks.length === 0 && isNew ? (
                  <p className="mutedNote">Start adding blocks below to build your competition page.</p>
                ) : null}
                <BlockEditorCanvas />
              </div>
            </div>
