...
export async function fetchExperts(): Promise<{ intro: string; experts: Expert[] }> {
  const res = await fetch(BASE, { cache: "no-store" });
  if (!res.ok) throw new Error(await readApiError(res, "Failed to fetch experts"));
...
export async function createExpertApi(
  ...
  if (!res.ok) throw new Error(await readApiError(res, "Failed to create expert"));
  return (await res.json()) as Expert;
}
...
export async function updateExpertApi(
  ...
  if (!res.ok) throw new Error(await readApiError(res, "Failed to update expert"));
  return (await res.json()) as Expert;
}

export async function deleteExpertApi(id: string): Promise<void> {
  const res = await fetch(`${BASE}/${id}`, { method: "DELETE" });
  if (!res.ok) throw new Error(await readApiError(res, "Failed to delete expert"));
}
...
export async function updateExpertsIntroApi(intro: string): Promise<string> {
  ...
  if (!res.ok) throw new Error(await readApiError(res, "Failed to update experts intro"));
