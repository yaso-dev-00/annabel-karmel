...
import { createExpert, getAllExperts, getExpertsIntro } from "@/lib/admin/experts-store";
...
export async function GET() {
  const [experts, intro] = await Promise.all([getAllExperts(), getExpertsIntro()]);
  return NextResponse.json({ intro, experts }, { headers: NO_STORE_HEADERS });
}

export async function POST(request: Request) {
  try {
    const body = (await request.json()) as Omit<Expert, "id" | "created_at" | "updated_at">;
    const expert = await createExpert(body);
    revalidateExpertPages(expert);
    return NextResponse.json(expert, { status: 201, headers: NO_STORE_HEADERS });
  } catch (error) {
