...
const LEGACY_BLOB_PATHNAME = 'cms/experts.json';
const VERSIONED_PREFIX = 'cms/experts/v';
const MAX_VERSIONS = 12;

const CMS_DIR = path.join(process.cwd(), 'data', 'cms');
const LOCAL_RUNTIME_FILE = path.join(CMS_DIR, 'experts.json');
const SEED_FILE = path.join(CMS_DIR, 'experts.seed.json');
const BUNDLED_SEED_RAW = JSON.stringify(seedStore);

export function isBlobExpertsCmsStoreEnabled(): boolean {
...
async function listExpertsStoreBlobs() {
...
async function readBlobRaw(): Promise<string | null> {
  if (!isBlobExpertsCmsStoreEnabled()) return null;
...
async function writeBlobRaw(raw: string): Promise<void> {
  if (!isBlobExpertsCmsStoreEnabled()) {
    throw new Error(
...
async function blobStoreHasData(): Promise<boolean> {
  if (!isBlobExpertsCmsStoreEnabled()) return false;
  const blobs = await listExpertsStoreBlobs();
  return blobs.length > 0;
}

export async function readExpertsCmsStoreRaw(): Promise<string> {
  if (isBlobExpertsCmsStoreEnabled()) {
...
export async function writeExpertsCmsStoreRaw(raw: string): Promise<void> {
  if (isBlobExpertsCmsStoreEnabled()) {
