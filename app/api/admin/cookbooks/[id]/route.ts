...
export async function PUT(request: Request, context: RouteContext) {
  try {
    const { id } = await context.params;
    const body = (await request.json()) as Partial<Omit<Cookbook, "id" | "created_at">>;
    ...
    const message = error instanceof Error ? error.message : "Failed to update cookbook";
...
export async function DELETE(_request: Request, context: RouteContext) {
  try {
    ...
    const message = error instanceof Error ? error.message : "Failed to delete cookbook";
