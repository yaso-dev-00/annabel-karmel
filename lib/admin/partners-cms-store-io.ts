...
export function useBlobPartnersCmsStore(): boolean {
...
async function readBlobRaw(): Promise<string | null> {
  if (!useBlobPartnersCmsStore()) return null;
...
async function blobStoreHasData(): Promise<boolean> {
  if (!useBlobPartnersCmsStore()) return false;
