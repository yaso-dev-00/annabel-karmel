...
export async function POST(request: Request) {
  try {
    const body = (await request.json()) as Omit<Cookbook, "id" | "created_at" | "updated_at">;
    const cookbook = await createCookbook(body);
    revalidateCookbookPages(cookbook);
    return NextResponse.json(cookbook, { status: 201, headers: NO_STORE_HEADERS });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Failed to create cookbook";
