...
export function useBlobProductsCmsStore(): boolean {
...
async function readBlobRaw(): Promise<string | null> {
  if (!useBlobProductsCmsStore()) return null;
...
async function blobStoreHasData(): Promise<boolean> {
  if (!useBlobProductsCmsStore()) return false;
